...
def remove_Dataframe_duplicates(dataframe, unique_values_in_columns=['UserID', 'ItemID'],
...
def load_CSV_into_SparseBuilder(filePath, header=False, separator="::", timestamp=False, remove_duplicates=False,
    ...
    if preinitialized_row_mapper is not None or preinitialized_col_mapper is not None:
        ...
        URM_all_builder = IncrementalSparseMatrix(auto_create_col_mapper=create_mapper,
                                                  auto_create_row_mapper=create_mapper)
        URM_timestamp_builder = IncrementalSparseMatrix(auto_create_col_mapper=create_mapper,
                                                        auto_create_row_mapper=create_mapper)
...
def remove_features(ICM, min_occurrence=5, max_percentage_occurrence=0.30, reconcile_mapper=None):
    ...
    feature_mask = np.logical_and(numOccurrences >= min_occurrence,
