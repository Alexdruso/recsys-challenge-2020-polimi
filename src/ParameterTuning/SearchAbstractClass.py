...
class SearchAbstractClass(object):
    ...
    def _set_search_attributes(self, recommender_input_args,
        ...
        if save_model not in self._SAVE_MODEL_VALUES:
            raise ValueError("{}: parameter save_model must be in '{}', provided was '{}'.".format(self.ALGORITHM_NAME,
        ...
        if save_model == "last" and recommender_input_args_last_test is None:
            self._write_log(
                "{}: parameter save_model is 'last' but no recommender_input_args_last_test provided, saving best model on train data alone.".format(
                    self.ALGORITHM_NAME))
            save_model = "best"
