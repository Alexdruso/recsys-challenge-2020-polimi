...
def compute_k_fold_significance(list_1, *other_lists):
    ...
    for other_list_index in range(len(other_lists)):
        ...
        assert isinstance(other_list, list) or isinstance(other_list,
                                                          np.ndarray), "The provided lists must be either Python lists or numpy.ndarray"
        assert len(list_1) == len(
            other_list), "The provided lists have different length, list 1: {}, list 2: {}".format(len(list_1),
                                                                                                   len(other_list))
