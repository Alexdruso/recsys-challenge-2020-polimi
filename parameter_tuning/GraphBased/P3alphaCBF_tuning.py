...
recommender = P3alphaCBFRecommender(URM_train=URM_train, ICM_train=ICM_all, verbose=False)
...
logger = JSONLogger(path="logs/" + recommender.RECOMMENDER_NAME + "_CBF_logs.json")
...
recommender = P3alphaCBFRecommender(URM_train=URM_all,ICM_train=ICM_all, verbose=False)
recommender.fit(topK=int(hyperparameters['topK']), alpha=hyperparameters['alpha'], implicit=True)

recommender.save_model(folder_path='../../models/', file_name=recommender.RECOMMENDER_NAME + '_CBF')
